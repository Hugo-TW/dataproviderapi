...
class StockerInfoSha(BaseType):
    ...
    def getData(self):
        try:
            ...
            keys = master1.keys( self.__COMPANYCODE + '-' + self.__SITE + '-' + self.__MAPID + '-' + self.__TYPE + '-SHA')
            if not keys :
            ...
            return {'SHA':data},200,{"Content-Type": "application/json",'Connection':'close','Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'POST','Access-Control-Allow-Headers':'x-requested-with,content-type'}
