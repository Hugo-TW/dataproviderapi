    ...
    def getData(self):
        try:
            ...
            sql =  """select * from 
                        ...
                        select to_char(t.data_date,'MON','nls_date_language=english')as month_date,
                        t.data_type,
                        to_char(round(decode(t.data_type,'COLOR',round(sum(t.data_value)/count(*),0),sum(t.data_value)/count(*)),1)) as data_value
                        from WAYNE_TEST_TV t
                        where item_name = 'SCRAP' 
                        and t.data_date >= to_date(substr('{1}',0,6)||'01','yyyy/mm/dd')
                        group by to_char(t.data_date,'MON','nls_date_language=english'),t.data_type
                        )
                        PIVOT (SUM (data_value)FOR data_type IN ('{2}')) 
                        order by decode(data_date,'JAN',1,'FEB',2,'MAR',3,'APR',4,'MAY',5,'JUN',6,'JUL',7,'AUG',8 ,'SEP',9,'OCT',10,'NOV',11,'DEC',12,13),data_date""".format(self.__start_time, self.__end_time, self.__sColnumName) 
