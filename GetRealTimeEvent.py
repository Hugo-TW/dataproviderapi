...
class RealTimeEvent(BaseType):
    def __init__(self, COMPANYCODE, SITE, MAPID, MESSAGETYPE):
        super().__init__()
    ...
    def getData(self):
        try:
            self.writeLog(f'{self.__class__.__name__} {sys._getframe().f_code.co_name}')
            key =  self.__COMPANYCODE + '-' + self.__SITE + '-' + self.__MAPID + '-' + self.__MESSAGETYPE
            self.getRedisConnection()
            if not self.searchRedisKeys(key):
                self.writeLog(f'{key} is not exist in redis')
                return {'Result': 'NG','Reason':'Key in redis is not exist'}, 200,{"Content-Type": "application/json",'Connection':'close','Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'POST','Access-Control-Allow-Headers':'x-requested-with,content-type'}
            data = self.getRedisData(key)        
