...
class AgvInfo(BaseType):
    ...
    def getData(self):
        try:
            ...
            sql =  "with position_mapping as (select t.item5 as Line, t.item6 as MM, t.item7 as SS, t.item8 as S, t.item9 as STK_POSITION,t.item10 as LINE_POSITION from DCSMOD3_DB.DCS_EQ_PORT_LIST t)select t.agv_id ,t1.Line||' ('||t.localtion_to||')' as location,to_date(to_char(t.create_date,'yyyy/mm/dd hh24miss'),'yyyy/mm/dd hh24miss') as start_time,t.create_date + (t1.s/86400) as end_time  from ( select Convert(t.event_msg, 'ZHT16MSWIN950', 'UTF8' ) as description,t.agv_id,t.localtion_from,t.localtion_to, t.current_position, max(t.create_date) over (partition by t.line_id,t.agv_id) as max_create_date,t.create_date from mcs_dispatch_command_log t where Convert(t.event_msg, 'ZHT16MSWIN950', 'UTF8' )  in ('3:AGVC已派車') and t.create_date is not null and t.create_user <> 'AGVC' and t.current_position in (select t.STK_POSITION from position_mapping t ) )t, position_mapping t1   where t.max_create_date = t.create_date and t.localtion_to = t1.LINE_POSITION(+) order by 1 asc"
            self.writeLog(f'SQL:\n {sql}')
