...
class wipLogFuncNp(BaseType):
    ...
    def getData( self ):
        try:           
            ...
            tmpCondition = copy.deepcopy(self.jsonData["CONDITION"])
            for k,v in tmpCondition.items():
                if len(v) != 0:
                    tmp.append(v)
                    tCONDITION[f'_id.{k}'] = v
            ...
            self.setMongoCollection("wipHisAndCurrentNoProduct")
            data = []
            #self.writeLog(f'CONDITION:{CONDITION}')
            m_data = self.aggregate(CONDITION)
            for m in m_data:
                #m["_id"] = str(m["_id"])
                for k,v in m['_id'].items():
                    m[k] = v
                del m['_id']
                m['ACCT_DATE'] = str(datetime.datetime.utcfromtimestamp(m['UTC']) + timedelta(hours=8)).split(' ')[0]
                data.append(m)   
