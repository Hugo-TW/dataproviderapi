    ...
    def getData(self):
        try:
            ...
            sql =  """select * from 
                        ...
                        FROM WAYNE_TEST_TV t 
                        where item_name = 'FPY_Y' 
                        and t.data_date between to_date('{0}','yyyy/mm/dd hh24miss') and to_date('{1}','yyyy/mm/dd hh24miss')

                        union
                        
                        select decode(to_char(t.data_date,'mm'),'01','Jan','02','Feb','03','Mar','04','Apr','05','May','06','Jun','07','Jul','08','Aug' ,'09','Sep','10','Oct','11','Nov','12','Dec')as month_date,
                        t.data_type,to_char(round(decode(t.data_type,'燈號',round(sum(t.data_value)/count(*),0),sum(t.data_value)/count(*)),1)) as data_value
                        from WAYNE_TEST_TV t
                        where item_name = 'FPY_Y' 
                        and t.data_date < to_date(substr('{0}',0,6)||'01','yyyy/mm/dd')
                        group by to_char(t.data_date,'mm'),t.data_type
                        )
                        PIVOT (SUM (data_value)FOR data_type IN ('{2}')) 
                        order by decode(data_date,'Jan',1,'Feb',2,'Mar',3,'Ap',4,'May',5,'Jun',6,'Jul',7,'Aug',8 ,'Sep',9,'Oct',10,'Nov',11,'Dec',12,13),data_date """.format(self.__start_time, self.__end_time, self.__sColnumName) 
